...
public class GameClient : MonoBehaviour
{
    ...
    public Button exitButton; // RENAMED from restartButton
    ...
    void HandleInput()
    {
        ...
        if (autoPilot)
        {
            // === AUTOMATIC MOVEMENT (BOT) ===
            // Move back and forth using a Sine wave
            // Speed = 3.0f, Magnitude > 0.5 to trigger input
            ...
            // Occasionally move Y to not get stuck
    ...
    void ParseMessage(string line)
    {
        ...
        else if (cmd == "SHUTDOWN")
        {
            ...
            // This line stops the game if you are testing inside the Unity Editor
            #if UNITY_EDITOR
    ...
    {
        if (mapGenerated) return; // Prevent double build
        ...
        for (int y = 0; y < h; y++)
        {
            for (int x = 0; x < w; x++)
            {
                ...
                {
                    // Walls still need +0.5f to center them on grid lines
        ...
        {
            // FIX: DO NOT ADD 0.5f HERE. Server already sends centered floats (e.g. 1.5).
